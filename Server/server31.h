...
/*--------------------------------------------------------------------------------------------*/
// DB 09/10/2007 Changed pointers to UINTs for 64-bit machine compatibility
...
// First argument should be a valid 1D spectrum
	...
	if (dt == keWord) {
		nz = 2;
		psWord = (UINT16*)pstorage;
	}
	if (dt == keLong) {
		nz = 4;
		psLong = (UINT32*)pstorage;
	}
	if (nz == 0) {
...
// DB 07/20/2007: In SpecTcl 3.1 the rResult is indirect and not that of the interpreter
//	objPtr = Tcl_GetObjResult(interp);
	objPtr = rResult.getObject();
...
/*-------------------------------------------------------------
	...
	This command returns a list containing the data of a 1D spectrum.
---------------------------------------------------------------*/
// DB 09/10/2007 Added support for 32 bit 2D spectra
...
// 	- mode=1: list of non-zero bins followed by list of their values
	...
	if (dt == keLong) {
		for (int y=0; y<ny; y++) {
			ynx = y * nx;
			for (int x=0; x<nx; x++) if (psLong[ynx + x] != 0) nonzero++;
		}
	}
	UINT32		zbytes = (nz+4) * nonzero;
// if there are less bytes using bins rather than indexed
// DB 07/20/2007: this option is still buggy in SpecTk so disable for now
/*
	if (nbytes < zbytes) {
		Tcl_SetByteArrayLength(objPtr, nbytes+1);
		data = Tcl_GetByteArrayFromObj(objPtr, NULL);
			...
			for (int y=0; y<ny; y++) {
				...
				for (int x=0; x<nx; x++) {
					if (psLong[x + ynx] != 0) {
						...
						data[j] = (char)y&0xFF;
						data[j+1] = (char)(y>>8);
						data[k] = (char)psLong[x + ynx]&0xFF;
						data[k+1] = (char)(psLong[x + ynx]>>8);
						data[k+2] = (char)(psLong[x + ynx]>>16);
						data[k+3] = (char)(psLong[x + ynx]>>24);
						i += 2;
						j += 2;
						k += 4;
					}
