...
//	iBlock->pixelPtr = (unsigned char*)realloc(iBlock->pixelPtr, sizeof(unsigned char)*pw*ph*3);
//	iBlock->pixelPtr = new unsigned char[pw*ph*4];
// Set the Tk_PhotoImageBlock structure
	...
	iBlock->offset[2] = 2;
	iBlock->offset[3] = 0;
//	Tk_PhotoGetImage(handle, iBlock);
	if (background == NULL) {
...
//				iBlock->pixelPtr[pxadd] = 255;
			}
		}
	}

// Put the image data into the photo image
//	Tk_PhotoSetSize(handle, 0, 0);
# if TCL_MINOR_VERSION==3
...
# endif
//	Tk_PhotoGetImage(handle, image);
	
// Clean up
	free(iBlock->pixelPtr);
	free(iBlock);
	for (ic=0; ic<colors; ic++) Tk_FreeColor(palette[ic]);
	free(palette);
	free(thresholds);
	free(data);
